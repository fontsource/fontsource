...
```javascript
import "@fontsource/port-lligat-slab/500.css" // Weight 500.
import "@fontsource/port-lligat-slab/900-italic.css" // Italic variant.
```

Alternatively, the same solutions could be imported via SCSS!

```scss
@import "~@fontsource/port-lligat-slab/index.css"; // Weight 400.
...
```scss
@import "~@fontsource/port-lligat-slab/scss/mixins";

// Uses a unicode-range map to automatically generate multiple @font-face rules.
@include fontFace(
  $weight: 500,
  $display: fallback,
  $fontDir: "~@fontsource/port-lligat-slab/files"
);
...
@include fontFaceCustom(
  ...
  $display: optional,
  $woff2Path:
    "~@fontsource/port-lligat-slab/files/port-lligat-slab-latin-500-normal.woff2",
  $unicodeRange: false
);
// More options available in link below.
```

You can see all of the existing inputtable mixin variables [here](https://github.com/fontsource/fontsource/tree/master/packages/port-lligat-slab/scss/mixins.scss).
...
```javascript
import "@fontsource/port-lligat-slab/latin-ext.css" // All weights with normal style included.
import "@fontsource/port-lligat-slab/cyrillic-ext-500.css" // Weight 500 with normal style.
import "@fontsource/port-lligat-slab/greek-900-italic.css" // Italic variant.
```
