...
```scss
@import "~@fontsource/turret-road/scss/mixins";

// Uses a unicode-range map to automatically generate multiple @font-face rules.
@include fontFace(
  $weight: 500,
  $display: fallback,
  $fontDir: "~@fontsource/turret-road/files"
);
...
@include fontFaceCustom(
  ...
  $display: optional,
  $woff2Path:
    "~@fontsource/turret-road/files/turret-road-latin-500-normal.woff2",
  $unicodeRange: false
);
// More options available in link below.
```

You can see all of the existing inputtable mixin variables [here](https://github.com/fontsource/fontsource/tree/master/packages/turret-road/scss/mixins.scss).
...
import "@fontsource/turret-road/greek-900-italic.css" // Italic variant.
```
