...
```javascript
import "@fontsource/krub/500.css" // Weight 500.
import "@fontsource/krub/900-italic.css" // Italic variant.
```

Alternatively, the same solutions could be imported via SCSS!

```scss
@import "~@fontsource/krub/index.css"; // Weight 400.
...
```scss
@import "~@fontsource/krub/scss/mixins";

// Uses a unicode-range map to automatically generate multiple @font-face rules.
@include fontFace(
  $weight: 500,
  $display: fallback,
  $fontDir: "~@fontsource/krub/files"
);
...
@include fontFaceCustom(
  $weight: 600,
  $display: optional,
  $woff2Path: "~@fontsource/krub/files/krub-latin-500-normal.woff2",
  $unicodeRange: false
);
// More options available in link below.
```

You can see all of the existing inputtable mixin variables [here](https://github.com/fontsource/fontsource/tree/master/packages/krub/scss/mixins.scss).
...
```javascript
import "@fontsource/krub/latin-ext.css" // All weights with normal style included.
import "@fontsource/krub/cyrillic-ext-500.css" // Weight 500 with normal style.
import "@fontsource/krub/greek-900-italic.css" // Italic variant.
```
